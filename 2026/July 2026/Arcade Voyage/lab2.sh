...
echo "${CYAN}${BOLD}           PERKVERSE - STARTING EXECUTION...${RESET}"
echo "${CYAN}${BOLD}==================================================================${RESET}"
echo
...
echo
echo "${ACTION_COLOR}${BOLD_TEXT}  Setting 10-second retention policy${RESET_FORMAT}"
...
echo
echo "${ACTION_COLOR}${BOLD_TEXT}  Setting temporary hold on file${RESET_FORMAT}"
gsutil retention temp set "gs://$BUCKET/dummy_transactions"
echo "${SUCCESS_COLOR}✓ Temporary hold applied${RESET_FORMAT}"

echo
echo "${ACTION_COLOR}${BOLD_TEXT}  Attempting file deletion (should fail)${RESET_FORMAT}"
gsutil rm "gs://$BUCKET/dummy_transactions"
echo "${WARNING_COLOR}⚠ Expected error occurred (file protected by hold)${RESET_FORMAT}"

echo
echo "${ACTION_COLOR}${BOLD_TEXT}  Releasing temporary hold${RESET_FORMAT}"
gsutil retention temp release "gs://$BUCKET/dummy_transactions"
echo "${SUCCESS_COLOR}✓ Hold released successfully${RESET_FORMAT}"

echo
echo "${ACTION_COLOR}${BOLD_TEXT}  Setting event-based hold as default${RESET_FORMAT}"
...
echo "${YELLOW}${BOLD}Thanks for using PerkVerse Lab Automation Scripts.${RESET}"
echo "${MAGENTA}${BOLD}Subscribe for more Google Cloud Arcade Lab Solutions.${RESET}"
echo
echo "${RED}${BOLD}https://www.youtube.com/@PerkVers${RESET}"
echo
